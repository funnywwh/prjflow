...
系统提供了从禅道（Zentao）到本系统的数据迁移工具，支持迁移以下数据：

- **部门迁移**：zt_dept -> departments
- **角色迁移**：zt_group + zt_grouppriv -> roles + permissions
- **用户迁移**：zt_user -> users
- **项目迁移**：zt_project -> projects
- **项目模块迁移**：zt_module -> modules
- **版本迁移**：zt_build -> versions
- **需求迁移**：zt_story -> requirements
- **任务迁移**：zt_task -> tasks
- **Bug迁移**：zt_bug -> bugs
- **项目成员迁移**：zt_team -> project_members

迁移工具会维护ID映射关系，确保外键关联正确。详细使用说明请参考 [数据迁移工具文档](../backend/cmd/migrate/README.md)
