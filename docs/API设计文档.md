...
**字段说明**:
- `progress` (可选): 进度值（0-100），如果未提供且更新了工时，将根据实际工时/预估工时自动计算
- `estimated_hours` (可选): 预估工时（小时）
- `actual_hours` (可选): 实际工时（小时），更新时会自动创建资源分配记录
- `work_date` (可选): 工作日期（YYYY-MM-DD），用于资源分配，默认为任务的开始日期或今天

**说明**:
- 更新实际工时时，系统会自动创建或更新资源分配记录
- 如果更新了实际工时或预估工时，进度会自动计算：`进度 = 实际工时 / 预估工时 * 100`（范围0-100%）
- 如果手动设置了进度，则不会自动计算
